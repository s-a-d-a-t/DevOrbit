      ...
      <div className="board">
        ...
        <div className="widget w-12">
          ...
          <form onSubmit={add} className="form-row">
            <input style={{ flex: 2 }} placeholder="Skill (e.g. TypeScript)" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} />
            <input placeholder="Category (frontend, dsa…)" list="skill-cats" value={form.category} onChange={(e) => setForm({ ...form, category: e.target.value })} />
            <datalist id="skill-cats">{categories.map(([c]) => <option key={c} value={c} />)}</datalist>
            <select value={form.level} onChange={(e) => setForm({ ...form, level: e.target.value })}>
              {LEVELS.map((l) => <option key={l} value={l}>{l}</option>)}
            </select>
            <input type="number" min="0" max="100" value={form.progress} onChange={(e) => setForm({ ...form, progress: +e.target.value })} title="Progress %" style={{ flex: '0 0 90px' }} />
              ...
              {items.map((s) => (
                <div key={s.id} className="widget">
                  ...
                  <div className="row-between" style={{ margin: '16px 0 6px' }}>
                    ...
                    <strong style={{ fontFamily: 'var(--mono)', fontSize: 15 }}>{s.progress}%</strong>
                  </div>
                  <div className="progress"><div style={{ width: `${s.progress}%` }} /></div>
                  <input
                    type="range" min="0" max="100" defaultValue={s.progress}
                  ...
                  <div className="row-between mt-8">
                    <select onChange={(e) => e.target.value && update(s, { projects: [...(s.projects || []), +e.target.value] })} value="" style={{ maxWidth: 180 }}>
